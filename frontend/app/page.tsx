...
import { PlusCircle, Brain, AlertTriangle, CheckCircle2, XCircle, Truck, Plane, Rocket, ChevronDown, Users } from 'lucide-react'; // Ícones
...
export default function OptimizePage() {
  ...
  const getRiskColorBorderBg = (riskLevel: string | undefined): string => {
    ...
    const lowerRisk = riskLevel.toLowerCase();
    if (lowerRisk.includes("muito alto")) return 'border-red-300 bg-red-50';
    if (lowerRisk.includes("alto")) return 'border-red-200 bg-red-50';
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 md:p-8 bg-slate-50 min-h-screen">
      ...
      <div className="mb-10 bg-white p-6 sm:p-8 rounded-xl shadow-xl border border-slate-200">
        ...
        {loadingDropdownData && <p className="text-center text-slate-500 animate-pulse py-4">Carregando opções de doadores e receptores...</p>}
      ...
      {result && !error && (
        <div className="mt-10 p-6 bg-white shadow-xl rounded-xl border border-slate-200 space-y-6">
          <div>
            <h2 className="text-2xl font-semibold text-slate-700 border-b pb-3 mb-4">Resultado da Otimização</h2>
            <p className="text-sm text-slate-600"><strong>Status:</strong> {result.message}</p>
            {result.logId && <p className="text-sm text-slate-600"><strong>ID do Log:</strong> {result.logId}</p>}
            {result.calculatedDistanceKm !== undefined && (
              <p className="text-sm text-slate-600"><strong>Distância Direta:</strong> {result.calculatedDistanceKm.toFixed(1)} km</p>
            )}
          </div>
          
          {result.overallRiskAssessment && (
            <div className={`p-3 rounded-md border text-sm font-medium ${getRiskColorBorderBg(result.overallRiskAssessment)}`}>
